...
- [x] 企业微信权限范围已落地：`data_policies` 只认 `WECOM/wecom` 来源，企业微信规则负责生成用户业务范围。
...
企业微信规则生成的用户数据范围统一写入 `business_scope.view`。合同、销售、结算单、收益地图先检查各自功能权限，再加载统一业务范围过滤门店、部门、柜组、供应商、品牌、品类。
...
| P0 | [x] 企业微信权限范围 | 按企业微信用户、部门、岗位和规则重建 `WECOM/wecom/business_scope.view` |
...
| [x] 业务员负责范围是柜组、供应商、品牌、楼层还是品类？ | 当前以企业微信规则同步到 `business_scope.view`，支持门店/部门/柜组等维度 |
