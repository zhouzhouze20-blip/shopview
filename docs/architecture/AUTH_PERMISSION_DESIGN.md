...
- `source_type`：固定为 `WECOM`
- `source_system`：固定为 `wecom`
...
合同、销售、结算单、收益地图都先判断自己的功能权限，再加载 `business_scope.view` 做数据过滤。用户业务数据范围只认企业微信来源策略：`source_type=WECOM/source_system=wecom`。
