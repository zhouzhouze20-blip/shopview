...
async def create_tenant(
    tenant: TenantCreate,
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("tenant.create")),
):
    """创建新租户"""
    # 检查租户编码是否已存在
...
async def update_tenant(
    ...
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("tenant.edit")),
):
    """更新租户信息"""
...
async def delete_tenant(
    tenant_id: int,
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("tenant.delete")),
):
    """删除租户（软删除）"""
    db_tenant = db.query(Tenant).filter(Tenant.tenant_id == tenant_id).first()
    if not db_tenant:
