...
async def get_counters(
    ...
    try:
        ...
        SELECT DISTINCT
            ...
            NULL::numeric as daily_revenue,
            NULL::numeric as revenue_per_sqm,
            NULL::text as revenue_trend,
            NULL::numeric as revenue_change_percent,
            -- 楼层信息
            {floor_select}
        FROM counters c
...
async def delete_counter(
    counter_id: int,
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("counter.delete")),
):
    """删除柜位（软删除）"""
    db_counter = db.query(Counter).filter(Counter.counter_id == counter_id).first()
    if not db_counter:
