...
async def create_floor(
    ...
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("floor.create")),
):
    """新增楼层字典"""
    try:
        cols = _ensure_building_area_column(db)
...
async def update_floor(
    ...
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("floor.edit")),
):
    """更新楼层字典"""
    try:
        cols = _ensure_building_area_column(db)
...
async def delete_floor(
    floor_id: int,
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("floor.delete")),
):
    """删除楼层字典"""
    _ensure_building_area_column(db)
    row = db.query(Floor).filter(Floor.id == floor_id).first()
