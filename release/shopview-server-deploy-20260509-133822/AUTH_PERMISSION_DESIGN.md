...
- `resource_code`：`business_scope` 用作统一业务数据范围；`counter`、`hall`、`tenant`、`contract`、`bill`、`revenue` 可保留为模块级数据范围扩展
- `action_code`：`view`、`edit`、`approve`、`export`
- `scope_mode`：`ALL`、`SELF`、`CUSTOM`
- `effect`：`ALLOW`、`DENY`
- `dimension_type`：`store`、`department`、`group`、`floor`、`unit`、`supplier`、`brand`、`category`
- `source_type`：`MANUAL`、`ERP`
- `source_system`：`shopview`、`erp`
...
合同、销售、结算单、收益地图都先判断自己的功能权限，再加载 `business_scope.view` 做数据过滤。ERP 用户-数据范围同步只维护 `source_type=ERP/source_system=erp` 的策略；系统页面手工配置只维护 `source_type=MANUAL/source_system=shopview` 的策略。
