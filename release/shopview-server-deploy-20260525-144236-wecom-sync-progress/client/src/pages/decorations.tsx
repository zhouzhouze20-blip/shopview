...
const ACTION_LABELS: Record<DecorationAction, string> = {
  edit: "编辑",
  submit: "提交审批",
  cancel: "取消项目",
  upload_attachment: "补附件",
  property_review: "物业审图",
  leader_approval: "领导审批",
  deposit_confirm: "保证金确认",
  entry_confirm: "进场确认",
  completion_submit: "提交完工",
  acceptance: "完工验收",
  settlement: "结算确认",
...
const dateStringFromNow = (days = 0) => {
  const value = new Date();
  value.setDate(value.getDate() + days);
  return value.toISOString().slice(0, 10);
};
const addDaysToDateString = (dateText: string, days: number) => {
  const value = new Date(`${dateText}T00:00:00`);
  value.setDate(value.getDate() + days);
  return value.toISOString().slice(0, 10);
};
const todayString = () => dateStringFromNow();
...
const defaultCreateForm = (storeId?: number | null): DecorationProjectCreateInput => {
  ...
  return {
    ...
    planned_finish_date: dateStringFromNow(14),
...
export default function DecorationsPage({ initialTab = "projects" }: { initialTab?: DecorationTab }) {
  ...
  const contractQuery = useContractDetail(createForm.contract_no);
  ...
  const stores = metaQuery.data?.stores ?? [];
  const departments = metaQuery.data?.departments ?? [];
  const detail = detailQuery.data ?? null;
  const visibleActions = (detail?.available_actions ?? []).filter((action) => action !== "edit" && action !== "upload_attachment");
  const hasApiError = Boolean(listQuery.error || todosQuery.error || detailQuery.error || workflowQuery.error || timelineQuery.error);
  ...
  useEffect(() => {
    ...
    const contractNo = params.get("contract_no")?.trim();
    if (!contractNo) return;
    setCreateForm((prev) => ({ ...prev, contract_no: contractNo }));
    setCreateOpen(true);
  ...
  useEffect(() => {
    const contractMain = contractQuery.data?.contmain;
    if (!contractMain) return;
    const startDate = contractMain.cmeffdate?.slice(0, 10) || "";
    setCreateForm((prev) => ({
      ...prev,
      brand_name: prev.brand_name || contractMain.cmppname || "",
      tenant_name: prev.tenant_name || contractMain.supplier_name || contractMain.cmtitle || "",
      planned_entry_date: startDate || prev.planned_entry_date,
      planned_finish_date: startDate && prev.planned_finish_date <= startDate
        ? addDaysToDateString(startDate, 14)
        : prev.planned_finish_date,
    }));
  }, [contractQuery.data]);
  ...
  const handleCreateStoreChange = (storeId: number) => {
    const nextDepartment = departments.find((item) => item.store_id === storeId);
    const nextForm = defaultCreateForm(storeId);
    setCreateForm((prev) => ({
      ...prev,
      store_id: storeId,
      department_id: nextDepartment?.id ?? prev.department_id,
      applied_area: nextForm.applied_area,
      spaces: nextForm.spaces,
    }));
  };
  ...
  const detailCards = useMemo(() => {
    if (!detail) return [];
    return [
      { label: "合同号", value: detail.contract_no ?? "-" },
  ...
  const handleCreateProject = async () => {
    try {
      if (!createForm.contract_no.trim()) {
        toast({ title: "请先选择或输入合同号", variant: "destructive" });
        return;
      }
            ...
            items.map((item) => (
              ...
              >
                <TableCell>
                  ...
                  <div className="text-xs text-slate-500">{item.store_name}{item.contract_no ? ` / ${item.contract_no}` : ""}</div>
                </TableCell>
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                从未开始合同发起装修项目，按进场审批、施工验收、退保证金流程闭环处理。
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            {isDecorationMockEnabled ? (
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-red-300 bg-red-50 px-3 py-2 text-xs text-red-700">
                <Sparkles className="h-3.5 w-3.5" />
                接口异常
              </div>
            ) : null}
                        ...
                        <TableBody>
                          {detail.spaces.map((space) => (
                            <TableRow key={space.id}>
                              ...
                              <TableCell>{SPACE_TYPE_LABELS[space.space_type] ?? space.space_type}</TableCell>
                          ...
                          workflow.nodes.map((node) => (
                            <div key={node.id} className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2">
                              <div>
                                ...
                                <div className="text-xs text-slate-500">{FLOW_TYPE_LABELS[workflow.flow_type] ?? workflow.flow_type}</div>
                              </div>
                              <Badge variant="outline">{NODE_STATUS_LABELS[node.status] ?? node.status}</Badge>
                            </div>
                          )),
                        )}
                      </div>
        ...
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2 md:col-span-2">
              <Label>合同号</Label>
              <Input
                value={createForm.contract_no}
                onChange={(e) => setCreateForm((prev) => ({ ...prev, contract_no: e.target.value.trim() }))}
                placeholder="输入未开始合同号，或从合同台账发起"
              />
              {createForm.contract_no.trim() ? (
                <div className={`text-xs ${contractQuery.error ? "text-red-600" : "text-slate-500"}`}>
                  {contractQuery.isLoading
                    ? "正在校验合同..."
                    : contractQuery.error
                      ? contractQuery.error instanceof Error ? contractQuery.error.message : "合同校验失败"
                      : contractQuery.data?.contmain
                        ? `合同状态：${contractQuery.data.contmain.status_label ?? "-"}，开始日期：${contractQuery.data.contmain.cmeffdate?.slice(0, 10) ?? "-"}`
                        : "创建时将校验合同必须尚未开始"}
                </div>
              ) : null}
            </div>
            <div className="space-y-2">
              <Label>门店</Label>
              <Select value={String(createForm.store_id)} onValueChange={(value) => handleCreateStoreChange(Number(value))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {stores.map((store) => (
            ...
            <div className="space-y-2">
              ...
              <Select value={String(createForm.department_id)} onValueChange={(value) => setCreateForm((prev) => ({ ...prev, department_id: Number(value) }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {departments.filter((item) => item.store_id === createForm.store_id).map((dept) => (
      ...
      <Dialog open={actionOpen} onOpenChange={setActionOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>执行动作：{pendingAction ? ACTION_LABELS[pendingAction] : ""}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            {renderActionFields()}
          </div>
