...
def generate_ai_report(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        if config["api_style"] == "responses":
            report = _call_responses_api(payload, config)
        else:
            report = _call_chat_completions_api(payload, config)
        return {
            "enabled": True,
...
def _call_responses_api(payload: dict[str, Any], config: dict[str, Any]) -> str | None:
    body = {
        "model": config["model"],
...
def _call_chat_completions_api(payload: dict[str, Any], config: dict[str, Any]) -> str | None:
