...
def _require_eligible_contract(db: Session, contract_no: str) -> dict[str, Any]:
    normalized = (contract_no or "").strip()
    if not normalized:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="请选择或输入合同号")
    if not _table_exists(db, "contmain"):
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="合同主表 contmain 未创建")

    row = db.execute(
        text(
            """
            SELECT cmcontno, cmstatus, cmeffdate::date AS cmeffdate, cmlapdate::date AS cmlapdate,
                   cmsupid, cmppname, cmtitle
            FROM contmain
            WHERE upper(trim(cmcontno)) = upper(trim(:contract_no))
            LIMIT 1
            """
        ),
        {"contract_no": normalized},
    ).mappings().first()
    if not row:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="合同不存在")

    start_date = row["cmeffdate"]
    if not start_date:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="合同缺少开始日期，不能创建装修项目")
    if start_date <= date.today():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="只有尚未开始的合同才能创建装修项目")

    existing = (
        db.query(DecorationProject)
        .filter(DecorationProject.contract_no == row["cmcontno"])
        .filter(~DecorationProject.current_status.in_(("COMPLETED", "REJECTED", "CANCELLED")))
        .first()
    )
    if existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"该合同已存在未完成装修项目: {existing.project_no}")
    ...
    if keyword:
        ...
        query = query.filter(
            or_(
                ...
                DecorationProject.contract_no.ilike(kw),
...
async def create_decoration(
    ...
    contract = _require_eligible_contract(db, payload.contract_no)
    project = DecorationProject(
        ...
        contract_no=contract["cmcontno"],
        contract_status=contract["cmstatus"],
        contract_start_date=contract["cmeffdate"],
        contract_end_date=contract["cmlapdate"],
