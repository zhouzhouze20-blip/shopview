...
async def update_counter_shape(
    ...
    try:
        ...
        # 更新柜位记录
        counter.shape_type = shape_data['shape_type']
        counter.polygon_coordinates = shape_data['polygon_coordinates']
        counter.center_x = shape_data['center_x']
        counter.center_y = shape_data['center_y']
        counter.bounding_box_min_x = shape_data['bounding_box_min_x']
        counter.bounding_box_min_y = shape_data['bounding_box_min_y']
        counter.bounding_box_max_x = shape_data['bounding_box_max_x']
        counter.bounding_box_max_y = shape_data['bounding_box_max_y']
...
async def get_counter_coordinates(
    ...
    try:
        ...
        # 解析坐标数据
        coordinates = []
        if counter.polygon_coordinates:
            coordinates = PolygonUtils.json_to_coordinates(counter.polygon_coordinates)
        ...
        return {
            ...
            "center": {
                "x": float(counter.center_x) if counter.center_x else None,
                "y": float(counter.center_y) if counter.center_y else None
            },
            "bounding_box": {
                "min_x": float(counter.bounding_box_min_x) if counter.bounding_box_min_x else None,
                "min_y": float(counter.bounding_box_min_y) if counter.bounding_box_min_y else None,
                "max_x": float(counter.bounding_box_max_x) if counter.bounding_box_max_x else None,
                "max_y": float(counter.bounding_box_max_y) if counter.bounding_box_max_y else None
            },
