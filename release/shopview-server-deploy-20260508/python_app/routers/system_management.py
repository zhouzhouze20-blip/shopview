...
async def get_contract_permission_options(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    departments = db.query(Department).filter(Department.is_active == True).order_by(Department.store_id.asc(), Department.dept_code.asc()).all()
    groups = db.query(CounterGroup).filter(CounterGroup.is_active == True).order_by(CounterGroup.store_id.asc(), CounterGroup.group_code.asc()).all()
    return {
        "stores": [
