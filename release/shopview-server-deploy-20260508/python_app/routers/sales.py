...
async def group_tickets(
    ...
    附带：salepay 中付款方式 0500、0580 的 je 合计为 coupon_amount；
    ...
    if has_salepay:
        pay_join = """
        LEFT JOIN (
          SELECT billno, SUM(COALESCE(je, 0)) AS coupon_amount
          FROM salepay
          WHERE TRIM(paycode) IN ('0500', '0580')
          GROUP BY billno
        ) sp ON sp.billno::text = TRIM(BOTH FROM s.sglbillno::text)
        """
    head_join = ""
    if has_salehead and dj_kind_col:
        ...
        """
    coupon_expr = "COALESCE(MAX(sp.coupon_amount), 0)" if has_salepay else "0"
    if has_salehead and dj_kind_col:
        txn_expr = f"""
          MAX(
