...
export function useUnitMapVersions(floorId?: number, baseMapId?: number) {
  return useQuery({
    queryKey: ["unit-map-versions", floorId ?? "all", baseMapId ?? "all"],
    queryFn: () =>
...
export function useDeleteUnitMapVersion() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["unit-map-versions"] });
      qc.invalidateQueries({ queryKey: ["geo-elements"] });
    },
  });
}
