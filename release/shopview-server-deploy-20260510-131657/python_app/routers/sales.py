...
def _counter_group_join_sql(enabled: bool) -> str:
    if not enabled:
        return ""
    return (
        "LEFT JOIN counter_groups cg "
        "ON upper(trim(COALESCE(s.sglmfid, ''))) = upper(trim(COALESCE(cg.group_code, '')))"
    )
...
def _aggregate_group_rows_to_departments(
    ...
    result = sorted(departments.values(), key=lambda item: item["effective_sales"], reverse=True)
    for item in result:
...
async def commodity_sales_detail_departments(
    ...
    """商品销售明细部门下拉：直接取 counter_groups 的部门主数据。"""
    require_permission(db, current_user, "sales.view")
    if not _table_exists(db, "counter_groups"):
        return []
...
async def commodity_sales_detail_report(
    ...
    """ERP 633 商品销售明细：直接按 ERP SQL 口径汇总，使用 counter_groups 替换 VIEW_MFRAME_ALL。"""
    require_permission(db, current_user, "sales.view")
    scope = load_business_scope(db, current_user, fallback_resource_code="sales")
    ...
    if not _table_exists(db, "counter_groups"):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="柜组主数据表未创建: counter_groups",
        )
    ...
    if group_code:
        sale_filters += (
            ...
            "OR EXISTS ("
            "  SELECT 1 FROM counter_groups cg_filter "
    ...
    rows = _fetch_mappings(
        ...
        FROM agg a
        LEFT JOIN counter_groups cg ON upper(trim(COALESCE(a.sglmfid, ''))) = upper(trim(COALESCE(cg.group_code, '')))
        {mkt_join}
        {goods_join}
...
def _group_level_sales_rows(
    ...
    *,
    ...
    """
    table_name = _salegoodslist_table(db)
    has_counter_groups = _table_exists(db, "counter_groups")
    has_stores = _table_exists(db, "stores")
...
async def ticket_detail(
    ...
    if has_salehead and has_salegoods:
        goods_scope_join = (
            "LEFT JOIN counter_groups cg ON upper(trim(COALESCE(g.gz, ''))) = upper(trim(COALESCE(cg.group_code, '')))"
            if has_counter_groups
            else ""
        )
