...
async def get_contract_permission_options(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    group_rows = db.execute(
        text(
            ...
            SELECT
              ROW_NUMBER() OVER (ORDER BY mg.store_id ASC, mg.department_code ASC, mg.group_code ASC) AS group_id,
              mg.*,
              st.store_name
            FROM manaframe_groups mg
            LEFT JOIN stores st ON st.store_id = mg.store_id
            ORDER BY mg.store_id ASC, mg.department_code ASC, mg.group_code ASC
            """
        )
    ).mappings().all()
    scope_matrix = [
