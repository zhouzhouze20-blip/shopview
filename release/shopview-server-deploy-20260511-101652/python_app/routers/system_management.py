...
async def sync_departments_from_counter_groups(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    rows = (
        db.query(
            CounterGroup.store_id,
            CounterGroup.department_code,
            CounterGroup.department_name,
        )
        .filter(
...
async def get_contract_permission_options(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # 权限范围配置以 ERP 柜组主数据为准。部分同步批次会把 counter_groups.is_active
    # 全量置为 false，但合同/销售仍按这些柜组编码做范围过滤，因此这里不按该状态字段过滤。
    groups = (
        db.query(CounterGroup)
        .order_by(CounterGroup.store_id.asc(), CounterGroup.group_code.asc())
        .all()
    )
    rows_joined = (
        db.query(CounterGroup, Store.store_name)
        .outerjoin(Store, Store.store_id == CounterGroup.store_id)
        .order_by(CounterGroup.store_id.asc(), CounterGroup.department_code.asc(), CounterGroup.group_code.asc())
        .all()
    )
    # 与合同/销售口径一致：门店–部门–柜组来自同一数据源 counter_groups（ERP）。
    # 使用 LEFT JOIN：避免因门店主数据缺失或 INNER JOIN 条件导致整表被放空；仅展示当前可选门店范围内的柜组。
    scope_matrix = [
        {
            "store_id": cg.store_id,
            "store_code": str(cg.store_id),
            "store_name": (store_name or "").strip() or (f"门店#{cg.store_id}" if cg.store_id is not None else ""),
            "department_code": (cg.department_code or "").strip(),
            "department_name": (cg.department_name or "").strip(),
            "group_id": cg.group_id,
            "group_code": cg.group_code,
            "group_name": cg.group_name,
        }
        for cg, store_name in rows_joined
    ]
    return {
        "stores": [
