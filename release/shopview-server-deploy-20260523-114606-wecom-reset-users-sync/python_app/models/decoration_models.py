...
class DecorationProject(Base):
    ...
    contract_no = Column(String(100), nullable=False, comment="关联合同号")
    contract_status = Column(String(30), comment="创建时合同状态")
    contract_start_date = Column(Date, comment="创建时合同开始日期")
    contract_end_date = Column(Date, comment="创建时合同结束日期")
