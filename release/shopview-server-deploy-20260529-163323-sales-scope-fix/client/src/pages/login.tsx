...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        const result = await apiGet<{ status: "pending" | "success" | "failed"; error?: string }>(
          `/api/auth/wecom/status?state=${encodeURIComponent(wecomState)}`,
        );
        if (result.status === "success") {
          ...
          window.clearInterval(timer);
          setError(result.error || "企业微信登录失败，请重新扫码。");
        }
      } catch (err) {
        const message = err instanceof Error ? err.message : "";
        if (message.includes("400") || message.includes("401")) {
          window.clearInterval(timer);
          setError("企业微信登录状态已失效，请刷新二维码后重试。");
        }
      }
    }, 2000);
    return () => window.clearInterval(timer);
  }, [activeTab, refresh, wecomState]);
          ...
          <div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-6">
              ...
              <TabsContent value="wecom" className="mt-6">
                <div className="h-[390px] overflow-hidden rounded-lg border border-dashed border-teal-200 bg-teal-50/70 sm:h-[410px]">
                  {wecomLoginUrl ? (
                    <div className="relative h-full w-full overflow-hidden bg-white">
                      <iframe
                        ...
                        src={wecomLoginUrl}
                        scrolling="no"
                        className="absolute left-1/2 top-0 h-[650px] w-[760px] origin-top -translate-x-1/2 scale-[0.62] border-0 bg-white sm:scale-[0.64]"
                      />
                    </div>
                  ) : (
