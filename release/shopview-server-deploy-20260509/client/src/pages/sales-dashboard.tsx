  ...
  const ticketsTableTotals = useMemo(() => {
    ...
    return rows.reduce(
      (acc, row) => ({
        ...
        point: acc.point + Number(row.point ?? 0),
      }),
      {
        quantity: 0,
        effective_sales: 0,
                ...
                <TableBody>
                  {(ticketsQuery.data ?? []).map((row) => (
                    <TableRow key={`${row.billno}`} className="cursor-pointer hover:bg-slate-50" onClick={() => setSelectedBillno(`${row.billno}`)}>
                      ...
                      <TableCell className="py-2 text-right">{number(row.point)}</TableCell>
                      <TableCell className="py-2">{row.point_type?.trim() || "-"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                {(ticketsQuery.data ?? []).length > 0 && (
                  <TableFooter>
                    <TableRow className="hover:bg-muted/50">
                      ...
                      <TableCell className="py-2 text-right font-semibold tabular-nums">{number(ticketsTableTotals.point)}</TableCell>
                      <TableCell />
                    </TableRow>
                  </TableFooter>
                )}
