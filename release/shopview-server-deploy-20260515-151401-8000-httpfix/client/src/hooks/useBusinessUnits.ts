...
export function useBusinessUnits(params?: { floorId?: number; status?: string; keyword?: string }) {
  return useQuery({
    queryKey: ["business-units", params?.floorId ?? "all", params?.status ?? "all", params?.keyword ?? ""],
    queryFn: () => {
...
export function useDeleteBusinessUnit() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: number) => apiDelete<{ message: string; id: number }>(`/api/business-units/${id}`),
    onSuccess: async () => {
