...
import { BarChart3, Building2, ChevronRight, Download, FileText, Loader2, RefreshCw, Search } from "lucide-react";
...
type TicketSummary = {
  ...
  ticket_margin: number;
  /** 付款方式 0500、0580 的 je 合计（salepay） */
...
export default function SalesDashboardPage() {
  ...
  const ticketsQuery = useQuery<TicketSummary[]>({
    ...
    queryFn: () =>
      apiGet(`/api/sales/groups/${selectedGroup?.group_code}/tickets${buildQuery(ticketsQueryParams)}`),
  ...
  const ticketDetailQuery = useQuery<TicketDetail>({
    ...
    queryFn: () => apiGet(`/api/sales/tickets/${selectedBillno}`),
  ...
  const ticketsTableTotals = useMemo(() => {
    ...
    return rows.reduce(
      (acc, row) => ({
        ...
        effective_sales: acc.effective_sales + Number(row.effective_sales || 0),
        net_profit: acc.net_profit + Number(row.net_profit || 0),
        coupon_amount: acc.coupon_amount + Number(row.coupon_amount || 0),
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">卡券金额</TableHead>
                ...
                <TableBody>
                  {(ticketsQuery.data ?? []).map((row) => (
                    <TableRow key={`${row.billno}`} className="cursor-pointer hover:bg-slate-50" onClick={() => setSelectedBillno(`${row.billno}`)}>
                      ...
                      <TableCell className="py-2 text-right">{number(row.quantity)}</TableCell>
                      <TableCell className="py-2 text-right">{money(row.effective_sales)}</TableCell>
                      <TableCell className="py-2 text-right">{money(row.net_profit)}</TableCell>
                      <TableCell className="py-2 text-right">{percentRatio(row.ticket_margin)}%</TableCell>
                      <TableCell className="py-2 text-right">{money(row.coupon_amount)}</TableCell>
                      <TableCell className="py-2 text-right">{money(row.authorized_discount)}</TableCell>
                      <TableCell className="py-2 text-right">{money(row.mzk)}</TableCell>
                      <TableCell className="py-2 text-right">{money(row.lq)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                {(ticketsQuery.data ?? []).length > 0 && (
                  <TableFooter>
                    <TableRow className="hover:bg-muted/50">
                      ...
                      <TableCell className="py-2 text-right font-semibold tabular-nums">{money(ticketsTableTotals.coupon_amount)}</TableCell>
