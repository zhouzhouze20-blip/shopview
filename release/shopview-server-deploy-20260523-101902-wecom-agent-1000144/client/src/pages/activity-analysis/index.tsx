...
type CouponSummaryPivotRow = {
  coupon_type: string;
  source_name: string;
  first_flow_date: number | string | null;
  last_flow_date: number | string | null;
  flow_count: number;
  member_count: number;
  ticket_count: number;
  coupon_pay_amount: number;
  ...
  sources: Set<string>;
  actionAmounts: Record<string, number>;
};
...
const ACTION_COLUMN_ORDER = ["返券", "销售返券", "前台买券", "后台买券", "券消费", "券消费冲正", "券退货", "返券扣回", "券退货冲正"];
...
export default function ActivityAnalysisPage() {
  ...
  const couponSummaryRows = couponSummaryQuery.data || [];

  const couponSummaryPivot = useMemo(() => {
    const actionMeta = new Map<string, { key: string; label: string }>();
    const grouped = new Map<string, CouponSummaryPivotRow>();

    couponSummaryRows.forEach((row) => {
      const actionCode = String(row.action_code || "");
      const actionName = String(row.action_name || actionCode || "未知动作");
      const actionId = `${actionCode || actionName}:${actionName}`;
      if (!actionMeta.has(actionId)) {
        actionMeta.set(actionId, { key: `action_amount_${actionMeta.size}`, label: actionName });
      }

      const couponType = String(row.coupon_type || "未标识");
      const current = grouped.get(couponType) || {
        coupon_type: couponType,
        source_name: "",
        first_flow_date: row.first_flow_date,
      ...
      if (String(row.first_flow_date || "") < String(current.first_flow_date || row.first_flow_date || "")) {
        current.first_flow_date = row.first_flow_date;
      }
      if (String(row.last_flow_date || "") > String(current.last_flow_date || row.last_flow_date || "")) {
        current.last_flow_date = row.last_flow_date;
      }

      const meta = actionMeta.get(actionId);
      const actionAmount = toNumber(row.raw_flow_amount);
      if (meta) current.actionAmounts[meta.key] = toNumber(current.actionAmounts[meta.key]) + actionAmount;
      current.flow_count = toNumber(current.flow_count) + toNumber(row.flow_count);
      current.member_count = toNumber(current.member_count) + toNumber(row.member_count);
      current.ticket_count = toNumber(current.ticket_count) + toNumber(row.ticket_count);
      current.coupon_pay_amount = toNumber(current.coupon_pay_amount) + toNumber(row.coupon_pay_amount);
      current.balance_amount = toNumber(current.balance_amount) + actionAmount;
      grouped.set(couponType, current);
    });

    const actionColumns = Array.from(actionMeta.values()).sort((a, b) => {
      const ia = ACTION_COLUMN_ORDER.indexOf(a.label);
      const ib = ACTION_COLUMN_ORDER.indexOf(b.label);
    ...
    const rows = Array.from(grouped.values())
      .map((row) => {
        const plain: RowData = {
          ...
          source_name: Array.from(row.sources).join("、") || "未标识",
          first_flow_date: row.first_flow_date,
          last_flow_date: row.last_flow_date,
          flow_count: row.flow_count,
          member_count: row.member_count,
          ticket_count: row.ticket_count,
          coupon_pay_amount: row.coupon_pay_amount,
          balance_amount: row.balance_amount,
        };
        actionColumns.forEach(({ key }) => {
          plain[key] = toNumber(row.actionAmounts[key]);
        });
        return plain;
      })
      .sort((a, b) => Math.abs(toNumber(b.balance_amount)) - Math.abs(toNumber(a.balance_amount)));
              ...
              {couponSummaryQuery.isError ? (
                ...
                <SimpleTable
                  ...
                  columns={[
                    ...
                    ...couponSummaryPivot.actionColumns.map(({ key, label }) => [key, label, money] as TableColumn),
