-- 账号、权限、数据范围 PostgreSQL 草案
-- 说明：
-- 1. 该文件为设计草案，不建议直接在生产执行
-- 2. 目标是兼容现有 users / counter_groups / business_units
-- 3. 数据边界以 store + department + group 为主
...
ALTER TABLE users
  ...
  ADD COLUMN IF NOT EXISTS default_store_id INTEGER,
...
CREATE TABLE IF NOT EXISTS user_identities (
  ...
  CONSTRAINT ck_user_identities_type
    CHECK (identity_type IN ('password', 'wecom'))
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_user_identities_type_identifier
  ON user_identities(identity_type, identifier);
...
CREATE TABLE IF NOT EXISTS departments (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT uq_departments_store_code UNIQUE (store_id, dept_code)
);
...
CREATE INDEX IF NOT EXISTS ix_user_department_posts_user
  ON user_department_posts(user_id);

CREATE INDEX IF NOT EXISTS ix_user_department_posts_dept
  ON user_department_posts(department_id);
...
CREATE TABLE IF NOT EXISTS user_roles (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT uq_user_roles UNIQUE (user_id, role_id, store_id)
);
...
CREATE TABLE IF NOT EXISTS data_policies (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT ck_data_policies_subject_type
    CHECK (subject_type IN ('ROLE', 'USER')),
  CONSTRAINT ck_data_policies_scope_mode
    CHECK (scope_mode IN ('ALL', 'SELF', 'CUSTOM')),
  CONSTRAINT ck_data_policies_effect
    CHECK (effect IN ('ALLOW', 'DENY'))
);

CREATE INDEX IF NOT EXISTS ix_data_policies_subject
  ON data_policies(subject_type, subject_id);

CREATE INDEX IF NOT EXISTS ix_data_policies_resource
  ON data_policies(resource_code, action_code);
...
CREATE TABLE IF NOT EXISTS data_policy_items (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT ck_data_policy_items_dimension_type
    CHECK (dimension_type IN ('store', 'department', 'group', 'floor', 'unit'))
);

CREATE INDEX IF NOT EXISTS ix_data_policy_items_policy
  ON data_policy_items(policy_id);

CREATE INDEX IF NOT EXISTS ix_data_policy_items_dimension
  ON data_policy_items(dimension_type, dimension_value);
...
CREATE TABLE IF NOT EXISTS login_logs (
  ...
  user_agent TEXT,
  created_at TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT ck_login_logs_result
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_counter_groups_store_group_code
  ON counter_groups(store_id, group_code);

ALTER TABLE business_units
  ADD COLUMN IF NOT EXISTS store_id INTEGER REFERENCES stores(store_id) ON DELETE RESTRICT,
  ADD COLUMN IF NOT EXISTS department_id BIGINT REFERENCES departments(id) ON DELETE RESTRICT,
  ADD COLUMN IF NOT EXISTS group_code VARCHAR(20);

CREATE INDEX IF NOT EXISTS ix_business_units_store
  ON business_units(store_id);

CREATE INDEX IF NOT EXISTS ix_business_units_department
  ON business_units(department_id);

CREATE INDEX IF NOT EXISTS ix_business_units_group_code
  ON business_units(group_code);
...
INSERT INTO data_policies (
  subject_type, subject_id, resource_code, action_code, scope_mode, effect, priority, is_active
)
SELECT 'ROLE', r.id, res.resource_code, res.action_code, 'ALL', 'ALLOW', 1, TRUE
FROM roles r
CROSS JOIN (
  VALUES
    ('counter', 'view'),
    ...
    ('revenue', 'view'),
    ('revenue', 'export')
) AS res(resource_code, action_code)
WHERE r.role_code = 'super_admin'
ON CONFLICT DO NOTHING;

-- store_admin / dept_manager / group_manager / finance / viewer
-- 建议由后台配置页按实际门店、部门、柜组动态生成 CUSTOM 策略
