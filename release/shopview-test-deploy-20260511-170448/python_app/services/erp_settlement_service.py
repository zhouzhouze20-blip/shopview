...
def list_joint_settlements(
    ...
    *,
    ...
    - dept_prefix: 部门/柜组编码开头过滤；按结算单可解析出的有效 mfid 前缀匹配。
    ...
    if not scope_all_access and scope_group_allow is not None and has_paybatch:
        allow_vals = sorted(scope_group_allow)
        pb_scope_expr = "TRIM(UPPER(COALESCE(NULLIF(TRIM(pb.pbmfid::text), ''), '')))"
        pb_allow_in, pb_scope_params = _sql_expr_in_binds(pb_scope_expr, allow_vals, "pb_allow")
        pb_scope_conds = [
            "TRIM(COALESCE(pb.pbjsno, '')) <> ''",
            f"LENGTH({pb_scope_expr}) > 0",
            pb_allow_in,
        ]
        if scope_group_deny:
            pb_deny_in, pb_deny_params = _sql_expr_in_binds(
                pb_scope_expr,
                sorted(scope_group_deny),
                "pb_deny",
            )
            pb_scope_conds.append(f"NOT ({pb_deny_in})")
            pb_scope_params.update(pb_deny_params)

        dept_prefix_clean = (dept_prefix or "").strip().upper()
        if dept_prefix_clean:
            pb_scope_conds.append(f"{pb_scope_expr} LIKE :dept_prefix")
            params["dept_prefix"] = f"{dept_prefix_clean}%"

        params = {**params, **pb_scope_params}
        where_sql = " AND ".join(conds)
        pb_scope_where = " AND ".join(pb_scope_conds)

        count_row = db.execute(
            text(
                f"""
                WITH scoped_pb AS (
                  SELECT DISTINCT TRIM(pb.pbjsno) AS sshbillno
                  FROM paybatch pb
                  WHERE {pb_scope_where}
                )
                SELECT COUNT(*) AS n
                FROM scoped_pb sp
                INNER JOIN supsettlehead h ON TRIM(h.sshbillno) = sp.sshbillno
                WHERE {where_sql}
                """
            ),
            params,
        ...
        if has_det:
            select_sales = (
                "(SELECT COALESCE(SUM(s.sdtxssr), 0) "
                "FROM supsettledettot s "
                "WHERE s.sdtbillno = h.sshbillno) AS sales_revenue_sum"
            )
        if has_chg:
            select_fee = (
                "(SELECT COALESCE(SUM(f.sscmoney), 0) "
                "FROM supsetcharge f "
                "WHERE f.sscjsno = h.sshbillno) AS fee_sum"
            )

        mf_join = ""
        dept_display_select = "sp.pbmfid AS sphmf_department_display"
        if has_mf:
            mf_join = "LEFT JOIN manaframe mf_sph ON TRIM(mf_sph.mfcode) = TRIM(sp.pbmfid)"
            dept_display_select = """
              CASE WHEN mf_sph.mfcode IS NOT NULL THEN
                TRIM(sp.pbmfid) || ' ' || COALESCE(mf_sph.mfcname::text, '')
              ELSE TRIM(sp.pbmfid)
        ...
        rows = db.execute(
            text(
                f"""
                WITH scoped_pb AS (
                  SELECT DISTINCT ON (TRIM(pb.pbjsno))
                    TRIM(pb.pbjsno) AS sshbillno,
                    TRIM(pb.pbbillno) AS pbbillno,
                    TRIM(pb.pbmfid::text) AS pbmfid
                  FROM paybatch pb
                  WHERE {pb_scope_where}
                  ORDER BY TRIM(pb.pbjsno),
                           CASE WHEN TRIM(pb.pbbilltype) = 'J' THEN 0 ELSE 1 END,
    ...
    # 列表页只需要快速判断/展示付款批次柜组。避免逐行 lateral 解析 suppayhead，
    # 否则带数据权限的查询会在大表上触发 statement_timeout。
    h_group_upper_sql = "TRIM(UPPER(COALESCE(NULLIF(TRIM(h.sshmfid::text), ''), '')))"
    pb_group_upper_sql = "TRIM(UPPER(COALESCE(NULLIF(TRIM(pb_scope.pbmfid::text), ''), '')))"

    dept_prefix_clean = (dept_prefix or "").strip().upper()
    if dept_prefix_clean:
        dept_expr = f"{h_group_upper_sql} LIKE :dept_prefix"
        if has_paybatch:
            dept_expr = (
                f"({dept_expr} OR EXISTS ("
                "SELECT 1 FROM paybatch pb_scope "
                "WHERE TRIM(pb_scope.pbjsno) = TRIM(h.sshbillno) "
                f"AND {pb_group_upper_sql} LIKE :dept_prefix"
                "))"
            )
        conds.append(dept_expr)
        params["dept_prefix"] = f"{dept_prefix_clean}%"
    ...
    if not scope_all_access and scope_group_allow is not None:
        allow_vals = sorted(scope_group_allow)
        h_allow_in, allow_p = _sql_expr_in_binds(h_group_upper_sql, allow_vals, "sc_allow")
        if has_paybatch:
            pb_allow_in, pb_allow_p = _sql_expr_in_binds(pb_group_upper_sql, allow_vals, "sc_allow")
            allow_p.update(pb_allow_p)
            scope_sql += (
                f" AND ((LENGTH({h_group_upper_sql}) > 0 AND {h_allow_in}) "
                "OR EXISTS ("
                "SELECT 1 FROM paybatch pb_scope "
                "WHERE TRIM(pb_scope.pbjsno) = TRIM(h.sshbillno) "
                f"AND LENGTH({pb_group_upper_sql}) > 0 AND {pb_allow_in}"
                "))"
            )
        else:
            scope_sql += f" AND LENGTH({h_group_upper_sql}) > 0 AND ({h_allow_in})"
        scope_bind.update(allow_p)
    if not scope_all_access and scope_group_deny:
        deny_vals = sorted(scope_group_deny)
        h_deny_in, deny_p = _sql_expr_in_binds(h_group_upper_sql, deny_vals, "sc_deny")
        if has_paybatch:
            pb_deny_in, pb_deny_p = _sql_expr_in_binds(pb_group_upper_sql, deny_vals, "sc_deny")
            deny_p.update(pb_deny_p)
            scope_sql += (
                f" AND NOT ((LENGTH({h_group_upper_sql}) > 0 AND {h_deny_in}) "
                "OR EXISTS ("
                "SELECT 1 FROM paybatch pb_scope "
                "WHERE TRIM(pb_scope.pbjsno) = TRIM(h.sshbillno) "
                f"AND LENGTH({pb_group_upper_sql}) > 0 AND {pb_deny_in}"
                "))"
            )
        else:
            scope_sql += f" AND NOT ({h_deny_in})"
    ...
    list_mf_expr = "NULL::varchar"
    if has_paybatch:
        pb_join = """
            LEFT JOIN LATERAL (
              SELECT TRIM(pb.pbbillno) AS pbbillno, TRIM(pb.pbmfid::text) AS pbmfid
              FROM paybatch pb
              WHERE TRIM(pb.pbjsno) = TRIM(h.sshbillno)
        ...
        list_mf_expr = "COALESCE(NULLIF(TRIM(h.sshmfid::text), ''), NULLIF(TRIM(pb_hdr.pbmfid::text), ''))"
    else:
        list_mf_expr = "NULLIF(TRIM(h.sshmfid::text), '')"

    mf_join = ""
    sphmfid_select = f"TRIM({list_mf_expr}) AS sphmfid"
    sphmf_disp_select = f"TRIM({list_mf_expr}) AS sphmf_department_display"
    if has_mf:
        mf_join = f"""
            LEFT JOIN manaframe mf_sph ON TRIM(mf_sph.mfcode) = TRIM({list_mf_expr})
        """
        sphmf_disp_select = f"""
            CASE WHEN mf_sph.mfcode IS NOT NULL THEN
              TRIM({list_mf_expr}) || ' ' || COALESCE(mf_sph.mfcname::text, '')
            ELSE TRIM({list_mf_expr})
            END AS sphmf_department_display
        """
    ...
    # COUNT：仅扫描 supsettlehead + EXISTS 权限子句；列表展示字段另行补充。
