import { ArrowLeft, Filter, Link2, Search } from "lucide-react"
...
import {
  getPaymentBills,
  getPaymentBillSummary,
} from "@/lib/etl-data"
import { TablePagination } from "@/components/table-pagination"
import { PaymentBillInlineActions } from "@/components/document-actions"
...
export default async function ERPDocumentsPage({
  ...
  const [erpPage, summary] = await Promise.all([
    getPaymentBills(
      params.page,
      undefined,
      params.supplierName,
    ...
    getPaymentBillSummary(params.supplierName, params.yearMonth, params.status),
  ])
  const erpDocuments = erpPage.items
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-semibold text-foreground">ERP单据</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            当前页面接入 ETLKP.PAYMENT_BILL 付款单数据，并通过匹配表统计关联发票。
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="pb-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              <CardTitle className="text-base font-medium">ERP单据列表</CardTitle>
              <Badge variant="secondary">{erpPage.total} 条记录</Badge>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <form className="flex items-center gap-2" action="/documents/erp">
              ...
              <Button asChild variant="outline" size="icon">
                <a href="/documents/erp" aria-label="清空筛选">
        ...
        <CardContent>
                ...
                {erpDocuments.map((doc) => (
                  <TableRow key={doc.id}>
                    ...
                    <TableCell className="text-right">
                      <PaymentBillInlineActions document={doc} />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
          ...
          <TablePagination
            page={erpPage.page}
            totalPages={erpPage.totalPages}
            total={erpPage.total}
            pageSize={erpPage.pageSize}
            basePath="/documents/erp"
            query={{
