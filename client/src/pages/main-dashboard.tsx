...
import { canAccessModule, findFirstAccessibleModule } from "@/lib/module-permissions";
import {
  Building2,
  Users,
  FileText,
...
function SystemOverview({
  ...
  const canOpenSales = canOpenModule("sales-dashboard");
  const canOpenContracts = canOpenModule("contracts");
  const salesQuery = useSalesDashboardOverview(canOpenSales);
  const contractQuery = useContractDashboardSummary(canOpenContracts);
  const expiringListQuery = useExpiringContractsThisMonth(expiringDialogOpen && canOpenContracts);
  ...
  return (
    ...
    <div className="space-y-4 p-4 sm:space-y-6 sm:p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">以下为当前账号可访问模块的数据汇总，点击卡片进入对应模块。</p>
      </div>

      <div className="flex flex-col gap-4 sm:gap-6">
        <Card
          className={cn(
            "cursor-pointer border-2 border-transparent transition-colors hover:border-slate-300 hover:bg-slate-50/80",
            !canOpenSales && "hidden",
          )}
          onClick={goSales}
          role="button"
        ...
        {!canOpenSales && !canOpenContracts && (
          <Card>
            <CardContent className="py-8 text-sm text-muted-foreground">
              当前账号暂无经营概览可展示的业务模块，请在系统管理中为角色分配对应模块查看权限。
            </CardContent>
          </Card>
        )}
      </div>
...
export default function MainDashboard() {
  ...
  const handleModuleChange = async (moduleId: string) => {
    if (!canOpenModule(moduleId)) {
      toast({
        title: "无模块权限",
        description: `当前账号不能访问「${MODULE_LABELS[moduleId] || moduleId}」。`,
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-50 flex flex-col md:flex-row" data-testid="main-dashboard">
      ...
      <main className="min-h-0 min-w-0 flex-1 overflow-auto" data-testid="main-content">
