...
def _get_store_code(db: Session, store_id: int) -> Optional[str]:
    row = db.execute(
        text("SELECT store_code FROM stores WHERE store_id = :store_id"),
        {"store_id": store_id},
    ).fetchone()
    return row.store_code if row and row.store_code is not None else None


def _append_floor_store_filter(db: Session, sql: str, params: dict, floor_expr: str, store_id: Optional[int]) -> str:
    if store_id is None:
        return sql
    floor_cols = _get_table_columns(db, "floors")
    store_col = "store_id" if "store_id" in floor_cols else ("store_code" if "store_code" in floor_cols else None)
    if not store_col:
        return sql
    params["store_id_text"] = str(store_id)
    params["store_code_text"] = _get_store_code(db, store_id)
    return (
        sql
        + f"""
            AND EXISTS (
              SELECT 1 FROM floors f
              WHERE f.id = {floor_expr}
                AND (
                  f.{store_col}::text = :store_id_text
                  OR (:store_code_text IS NOT NULL AND f.{store_col}::text = :store_code_text)
                )
            )
        """
    )
...
async def list_business_units(
    ...
    try:
        ...
        sql = _append_floor_store_filter(db, sql, params, "business_units.floor_id", store_id)
...
async def create_business_unit(
    body: dict,
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("business_unit.create")),
):
    """新增经营单元。"""
    try:
...
async def update_business_unit(
    ...
    body: dict,
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("business_unit.edit")),
):
    """更新经营单元。"""
    try:
        current = db.execute(
...
async def delete_business_unit(
    unit_id: int,
    db: Session = Depends(get_db),
    _=Depends(require_permission_dependency("business_unit.delete")),
):
    """删除经营单元。"""
    try:
        row = db.execute(
            text("SELECT id, unit_code FROM business_units WHERE id = :id"),
            {"id": unit_id},
        ).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="经营单元不存在")

        if _table_exists(db, "decoration_project_spaces"):
            decoration_ref = db.execute(
                text("SELECT COUNT(*) AS cnt FROM decoration_project_spaces WHERE business_unit_id = :id"),
                {"id": unit_id},
            ).fetchone()
            if decoration_ref and int(decoration_ref.cnt) > 0:
                raise HTTPException(
                    status_code=400,
                    detail=f"经营单元 {row.unit_code} 已被装修项目引用，不能直接删除",
                )

        detached_binding_count = 0
        if _table_exists(db, "business_unit_binding"):
            binding_ref = db.execute(
                text("SELECT COUNT(*) AS cnt FROM business_unit_binding WHERE shop_unit_id = :id"),
                {"id": unit_id},
            ).fetchone()
            detached_binding_count = int(binding_ref.cnt) if binding_ref else 0
            if detached_binding_count > 0:
                db.execute(
                    text(
                        """
                        UPDATE business_unit_binding
                        SET shop_unit_id = NULL, updated_at = NOW()
                        WHERE shop_unit_id = :id
                        """
                    ),
                    {"id": unit_id},
                )
        ...
        geo_result = db.execute(text("DELETE FROM geo_elements WHERE unit_id = :id"), {"id": unit_id})
