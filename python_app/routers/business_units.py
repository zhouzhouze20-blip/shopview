...
def _expand_floor_filter_ids(db: Session, floor_id: int) -> list[int]:
    """
    兼容 floors/store_floors 双表 id 映射，用于列表筛选。
    """
    ids = {int(floor_id)}
...
def _resolve_target_floor_id(db: Session, floor_id: int) -> int:
    """
    将前端 floor_id 映射为 business_units.floor_id 可写入的目标 id。
    若无法映射，则返回原值，让后续显式报错。
    """
    target = _get_floor_fk_target_table(db)
    if target == "floors" or not target:
        return floor_id

    # 目标为 store_floors 时，优先按编码映射
    if target == "store_floors" and _table_exists(db, "floors") and _table_exists(db, "store_floors"):
        f = db.execute(
            text("SELECT building_code, floor_code FROM floors WHERE id = :id"),
            {"id": floor_id},
        ).fetchone()
...
async def create_business_unit(
    ...
    try:
        ...
        if target not in {"floors", "store_floors"}:
