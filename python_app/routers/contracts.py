...
def _normalize_store_ref(value: Any) -> str:
...
def _counter_group_scope_join_sql(enabled: bool, code_expr: str, alias: str = "cg") -> str:
    ...
    return (
        ...
        LEFT JOIN (
          ...
          LEFT JOIN stores st
            ON upper(trim(COALESCE(st.store_code, ''))) = upper(
              CASE
                WHEN SUBSTRING(TRIM(BOTH FROM COALESCE(mf.mfcode, '')) FROM 1 FOR 3) ~ '^[0-9]+$'
                THEN SUBSTRING(TRIM(BOTH FROM COALESCE(mf.mfcode, '')) FROM 1 FOR 3)
                ELSE ''
    ...
    try:
        ...
        unit = db.execute(
            text(
                ...
                LEFT JOIN stores st_by_floor_code
                  ON upper(trim(COALESCE(f.store_code, ''))) = upper(trim(COALESCE(st_by_floor_code.store_code, '')))
                LEFT JOIN stores st_by_sf
                  ON upper(trim(COALESCE(sf.store_id::varchar, ''))) = upper(trim(COALESCE(st_by_sf.store_id::varchar, '')))
                  OR upper(trim(COALESCE(sf.store_id::varchar, ''))) = upper(trim(COALESCE(st_by_sf.store_code, '')))
                WHERE bu.id = :unit_id
                """
            ),
            {"unit_id": unit_id},
        ...
        unit_store_refs = sorted(
            {
                ref
                for ref in (
                    _normalize_store_ref(unit.store_code),
                    _normalize_store_ref(unit.store_id),
                )
                if ref
            }
        )

        def unit_store_match_sql(store_expr: str) -> str:
            if not unit_store_refs:
                return "TRUE"
            return f"upper(trim(COALESCE(({store_expr})::varchar, ''))) = ANY(:unit_store_refs)"

        cmf_store_match = unit_store_match_sql("cmf.cmfmarket")
        contract_store_match = unit_store_match_sql("COALESCE(cmf.cmfmarket, cm.cmjsmkt)")
        binding_matches_cte = (
            """
                binding_matches AS (
                  SELECT
        ...
        rows = db.execute(
            ...
            {"unit_code": unit_code, "unit_id": unit_id, "unit_store_refs": unit_store_refs},
        ).mappings().all()
