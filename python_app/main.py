...
# 配置CORS（本地 Vite 直连后端时需放行；credentials=True 时不能使用 "*"）
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
        ...
        "http://192.168.98.81:5174",
        "http://192.168.98.81:7000",
...
async def wecom_domain_verify_file(verify_file: str):
    ...
    possible_paths = [
        ...
        PROJECT_ROOT / "static" / verify_file,
        Path("static") / verify_file,
        Path("/app") / verify_file,
    ]
    for verify_path in possible_paths:
