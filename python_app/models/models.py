...
class Floor(Base):
    ...
    __table_args__ = (UniqueConstraint("building_code", "floor_code", name="uq_floors_building_floor"),)
...
class StoreFloor(Base):
    ...
    halls = relationship("Hall", back_populates="floor")
...
class Counter(Base):
    ...
    store_id = Column(Integer, ForeignKey("stores.store_id"), nullable=False)
    floor_id = Column(Integer, ForeignKey("store_floors.floor_id"), nullable=False)
    ...
    floor = relationship("StoreFloor", back_populates="counters")
...
class Hall(Base):
    ...
    hall_code = Column(String(20), unique=True, nullable=False, comment="厅房编码")
    hall_name = Column(String(200), comment="厅房名称")
    store_id = Column(Integer, ForeignKey("stores.store_id"), nullable=False)
    floor_id = Column(Integer, ForeignKey("store_floors.floor_id"), nullable=False)
    counter_number = Column(String(20), nullable=False, comment="柜位编号")
    area = Column(Numeric(10, 2), comment="面积（平方米）")
    shape_type = Column(String(20), default="rectangle", comment="形状类型：rectangle/polygon")
    position_data = Column(JSON, comment="位置数据（坐标、尺寸等）")
    is_active = Column(Boolean, default=True, comment="是否启用")
    created_at = Column(DateTime, default=func.now(), comment="创建时间")
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now(), comment="更新时间")
    ...
    floor = relationship("StoreFloor", back_populates="halls")
    group_bindings = relationship("HallGroupBinding", back_populates="hall")
...
class CounterGroup(Base):
    ...
    group_code = Column(String(20), unique=True, nullable=False, comment="柜组编码（ERP系统）")
    ...
    # 关联关系
    store = relationship("Store")
    # hall_bindings = relationship("HallGroupBinding", back_populates="counter_group")


class HallGroupBinding(Base):
    """厅房-柜组关联表"""
    __tablename__ = "hall_group_bindings"

    binding_id = Column(Integer, primary_key=True, index=True)
    hall_id = Column(Integer, ForeignKey("halls.hall_id"), nullable=False)
    group_code = Column(String(20), nullable=False, comment="柜组编码")
    is_active = Column(Boolean, default=True, comment="是否启用")
    bound_at = Column(DateTime, default=func.now(), comment="绑定时间")
    bound_by = Column(String(50), comment="绑定操作人")

    # 关联关系
    hall = relationship("Hall", back_populates="group_bindings")
    # counter_group = relationship("CounterGroup", back_populates="hall_bindings", 
    #                             primaryjoin="HallGroupBinding.group_code == CounterGroup.group_code")
...
class RevenueData(Base):
    ...
    counter_id = Column(Integer, nullable=False, comment="柜位ID")
    counter_code = Column(String(20), comment="柜位编码")
    counter_name = Column(String(100), comment="柜位名称")
    store_id = Column(Integer, comment="门店ID")
    store_name = Column(String(100), comment="门店名称")
    floor_id = Column(Integer, comment="楼层ID")
    floor_name = Column(String(50), comment="楼层名称")
    area = Column(Numeric(10, 2), comment="面积（平方米）")
    x = Column(Numeric(10, 4), comment="X坐标")
    y = Column(Numeric(10, 4), comment="Y坐标")
    width = Column(Numeric(10, 4), comment="宽度")
    height = Column(Numeric(10, 4), comment="高度")
    monthly_revenue = Column(Numeric(10, 2), comment="月收益")
    daily_revenue = Column(Numeric(10, 2), comment="日收益")
    revenue_per_sqm = Column(Numeric(10, 2), comment="每平米收益")
    revenue_trend = Column(String(20), comment="收益趋势")
    revenue_change_percent = Column(Numeric(5, 2), comment="收益变化百分比")
    ...
    date = Column(Date, comment="完整日期")
    
    # 同期对比字段
    same_period_sales = Column(Numeric(10, 2), comment="同期销售")
    same_period_date = Column(Date, comment="同期日期")
    same_period_revenue = Column(Numeric(10, 2), comment="同期收益")
    year_over_year = Column(Numeric(5, 2), comment="同比（百分比）")
...
class DataPolicyItem(Base):
    ...
    dimension_type = Column(String(20), nullable=False, comment="维度类型：store, department, group, floor, unit")
...
class SalesProfit(Base):
    """销售毛利表"""
    __tablename__ = "sales_profits"
    ...
    # 销售数据
    gross_sales = Column(Numeric(12, 2), nullable=False, comment="销售总额")
    cost_of_goods = Column(Numeric(12, 2), nullable=False, comment="商品成本")
    gross_profit = Column(Numeric(12, 2), nullable=False, comment="毛利")
    profit_margin = Column(Numeric(5, 2), comment="毛利率（百分比）")
    
    # 其他信息
    product_category = Column(String(50), comment="商品类别")
    brand_name = Column(String(100), comment="品牌名称")
    sales_person = Column(String(50), comment="销售人员")
...
class Fee(Base):
    ...
    counter_id = Column(Integer, ForeignKey("counters.counter_id"), nullable=False, comment="柜位ID")
    fee_date = Column(Date, nullable=False, comment="收费日期")
    year = Column(Integer, comment="年份")
    month = Column(Integer, comment="月份")
    day = Column(Integer, comment="日期")
    
    # 收费数据
    fee_type = Column(String(50), nullable=False, comment="收费类型：租金/管理费/水电费/其他")
    fee_amount = Column(Numeric(12, 2), nullable=False, comment="收费金额")
    fee_description = Column(String(200), comment="收费说明")
    
    # 其他信息
    billing_period = Column(String(20), comment="计费周期")
    payment_status = Column(String(20), default="unpaid", comment="付款状态：unpaid-未付, paid-已付, overdue-逾期")
    payment_date = Column(Date, comment="付款日期")
    payment_method = Column(String(20), comment="付款方式")
    notes = Column(Text, comment="备注")
...
class Order(Base):
    ...
    order_type = Column(String(20), comment="订单类型：销售/退货/换货")
    order_status = Column(String(20), default="pending", comment="订单状态：pending-待处理, completed-已完成, cancelled-已取消")
...
class OrderItem(Base):
    ...
    total_price = Column(Numeric(12, 2), nullable=False, comment="总价")
    
    # 成本信息
    unit_cost = Column(Numeric(10, 2), comment="单位成本")
    total_cost = Column(Numeric(12, 2), comment="总成本")
    profit = Column(Numeric(12, 2), comment="利润")
